...
Pace yourself like a human: wait 3–5 seconds between page loads, never open more than one page at a time, and don't reload the same page repeatedly. If the site answers with HTTP 429 (rate limited), wait ~90 seconds once and continue; if it happens again, stop testing new pages and write the report with what you have — record it as a WARNING finding ("site rate-limits normal browsing").
