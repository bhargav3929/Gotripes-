...
<head>
    ...
    <title>Agency Login - GoTrips</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-group input {
            ...
            font-size: 14px; font-family: inherit; color: #f0f0f0;
            background: #1a1a1a; height: 42px;
            transition: border-color 0.2s, box-shadow 0.2s;
        }
        .form-group input:focus {
            outline: none; border-color: #FFD700;
            box-shadow: 0 0 0 2px rgba(255, 215, 0, 0.25); background: #222;
        }
        ...
        .login-btn {
            ...
            height: 42px; text-transform: uppercase; letter-spacing: 0.5px;
        }
        .login-btn:hover {
...
<body>
    ...
    <div class="login-card">
        @if($errors->has('email'))
            <div class="login-error">
                {{ $errors->first('email') }}
                @if(session('show_renewal_link'))
                    <br><a href="{{ route('agency.renew-license') }}" style="color:#FFD700;">Submit your renewed license &rarr;</a>
                @endif
            </div>
        @endif
    ...
    <div class="login-footer">
        <a href="{{ route('agency.register') }}">Apply to become a partner</a> ·
