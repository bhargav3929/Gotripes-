        ...
        <div class="card shadow">
            <div class="card-header">
                <div class="d-sm-flex align-items-center justify-content-between ">
                    <h1 class="h3 mb-0 text-gray-800">{{ __('Create Transaction') }}</h1>
                    <a href="{{ route('admin.transactions.index') }}" class="btn btn-primary btn-sm shadow-sm">{{ __('Go Back') }}</a>
                </div>
            </div>
            <div class="card-body">
            ...
            <div class="form-group row">
                <label class="col-sm-2 col-form-label"><strong>{{ __('Referee Name') }} :</strong><span style="color:red">*</span> </label>
                <div class="col-sm-10">
                    <select class="form-control" name="refereeID" id="refereeID" required>
                        ...
                        @foreach ($referees as $referee)
                            <option value="{{ $referee->id }}">{{ $referee->refereeName }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
