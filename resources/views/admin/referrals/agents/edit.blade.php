...
@section('title', 'Edit Referral Agent')
...
<div class="container-fluid py-3">
    ...
    <div class="row g-3">
        <div class="col-lg-8">
            <form method="POST" action="{{ route('admin.referrals.agents.update', $agent) }}">
                @csrf
                @method('PUT')
                            ...
                            <div class="col-12">
                                ...
                                <div class="input-group input-group-sm">
                                    ...
                                    <input type="text" name="referral_code" id="referralCode"
                                           ...
                                           value="{{ old('referral_code', $agent->referral_code) }}">
                                    <button type="button" class="btn btn-outline-info copy-btn" data-copy="{{ $agent->referral_url }}">
                        ...
                        <div class="row g-2">
                            <div class="col-md-6">
                                ...
                                <select name="commission_type" id="commissionType" class="form-select form-select-sm @error('commission_type') is-invalid @enderror" required>
                                    <option value="percentage" {{ old('commission_type', $agent->commission_type) == 'percentage' ? 'selected' : '' }}>Percentage (%)</option>
                                    <option value="fixed" {{ old('commission_type', $agent->commission_type) == 'fixed' ? 'selected' : '' }}>Fixed (AED)</option>
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group input-group-sm">
                                    <input type="number" name="commission_value" step="0.01" min="0"
                                           ...
                                           value="{{ old('commission_value', $agent->commission_value) }}" required>
                                    <span class="input-group-text" id="commissionSuffix">{{ $agent->commission_type === 'percentage' ? '%' : 'AED' }}</span>
                                </div>
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        <textarea name="notes" class="form-control form-control-sm @error('notes') is-invalid @enderror"
                                  rows="2">{{ old('notes', $agent->notes) }}</textarea>
        ...
        <div class="col-lg-4">
                    ...
                    <div class="row g-2">
                        <div class="col-6">
                            <div class="stat-box gold">
                                <div class="stat-value">{{ number_format($agent->total_clicks) }}</div>
                                <small class="text-muted">Clicks</small>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="stat-box info">
                                <div class="stat-value">{{ number_format($agent->total_sales) }}</div>
                                <small class="text-muted">Sales</small>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="stat-box success">
                                <div class="stat-value">{{ number_format($agent->total_earnings, 0) }}</div>
                                <small class="text-muted">Earned (AED)</small>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="stat-box warning">
                                <div class="stat-value">{{ number_format($agent->pending_earnings, 0) }}</div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="info-row"><span>Conversion</span><span class="text-success">{{ $agent->conversion_rate }}%</span></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content bg-dark">
            ...
            <div class="modal-body py-2">
                <p class="small mb-0">Delete <strong>{{ $agent->name }}</strong>? All referral data will be lost.</p>
            </div>
            <div class="modal-footer border-dark py-2">
                <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Cancel</button>
                <form method="POST" action="{{ route('admin.referrals.agents.destroy', $agent) }}">
                    @csrf @method('DELETE')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('regenerateCode').addEventListener('click', function() {
        ...
        fetch('{{ route("admin.referrals.agents.regenerate-code", $agent) }}', {
            method: 'POST',
