...
<div class="container-fluid py-3">
    ...
    <div class="d-flex justify-content-between align-items-start mb-3">
        <div class="d-flex align-items-center gap-2">
            <a href="{{ route('admin.referrals.dashboard') }}" class="btn btn-outline-secondary btn-sm">
    ...
    <div class="row g-2 mb-3">
        <div class="col-6 col-lg-3">
            <div class="stat-box stat-box-warning">
                ...
                <div class="stat-sub">AED {{ number_format($pendingTotal ?? 0, 2) }}</div>
            </div>
        </div>
        <div class="col-6 col-lg-3">
            <div class="stat-box stat-box-info">
                ...
                <div class="stat-value text-info">{{ $stats['processing_count'] ?? 0 }}</div>
                <div class="stat-sub">AED {{ number_format($processingTotal ?? 0, 2) }}</div>
            </div>
        </div>
        <div class="col-6 col-lg-3">
            <div class="stat-box stat-box-success">
                ...
                <div class="stat-value text-success">{{ $stats['completed_count'] ?? 0 }}</div>
                <div class="stat-sub">AED {{ number_format($completedTotal ?? 0, 2) }}</div>
            </div>
        </div>
        <div class="col-6 col-lg-3">
            <div class="stat-box stat-box-danger">
                ...
                <div class="stat-value text-danger">{{ $stats['rejected_count'] ?? 0 }}</div>
    ...
    <div class="card mb-3">
        <div class="card-body py-2">
            <form method="GET" action="{{ route('admin.referrals.withdrawals.index') }}" class="row g-2 align-items-end">
                ...
                <div class="col-6 col-md-5">
                    ...
                    <input type="text" name="search" class="form-control form-control-sm"
                           value="{{ request('search') }}" placeholder="Agent name or email...">
                </div>
                <div class="col-12 col-md-4 d-flex gap-1">
                    <button type="submit" class="btn btn-gold btn-sm flex-grow-1">
                        <i class="fas fa-search me-1"></i>Filter
                    </button>
                    <a href="{{ route('admin.referrals.withdrawals.index') }}" class="btn btn-outline-secondary btn-sm" title="Clear filters">
        ...
        <div class="table-responsive">
            <table class="table mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <div class="agent-name-text">{{ $withdrawal->referralAgent->name ?? 'N/A' }}</div>
                            <small class="text-muted">{{ $withdrawal->referralAgent->email ?? '' }}</small>
                        </td>
                        <td class="text-end">
                            <span class="withdrawal-amount">AED {{ number_format($withdrawal->amount, 2) }}</span>
                        </td>
                        <td>
                        ...
                        <td class="text-center">
                            ...
                            @endphp
                            <span class="badge bg-{{ $statusBadge }}">{{ ucfirst($withdrawal->status) }}</span>
                        </td>
                        <td>
                        ...
                        <td class="text-center">
                            <div class="d-flex align-items-center justify-content-center gap-1">
                                @if($withdrawal->status === 'pending')
                                    <!-- Mark Processing -->
                                    <form method="POST" action="{{ route('admin.referrals.withdrawals.approve', $withdrawal) }}">
                                        @csrf
                                ...
                                @elseif($withdrawal->status === 'processing')
                                    <!-- Mark Complete -->
                                    <form method="POST" action="{{ route('admin.referrals.withdrawals.complete', $withdrawal) }}">
                                        @csrf
                                ...
                                @elseif($withdrawal->status === 'rejected')
                                    <span class="text-danger" title="{{ $withdrawal->admin_notes ?? 'Rejected' }}">
                                        <i class="fas fa-times-circle"></i>
                                    </span>
                                    @if($withdrawal->admin_notes)
                                    <small class="text-muted" style="max-width: 120px; display: inline-block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{{ $withdrawal->admin_notes }}">{{ $withdrawal->admin_notes }}</small>
                                    @endif
                                @endif
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="text-center py-5">
                                ...
                                <a href="{{ route('admin.referrals.withdrawals.index') }}" class="btn btn-sm btn-outline-secondary mt-2" style="font-size: 0.7rem;">Clear filters</a>
                                @endif
                            </div>
                        </td>
                    </tr>
