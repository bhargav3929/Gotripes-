                    ...
                    <form method="POST" action="{{ route('admin.referrals.settings.update') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="type-toggle-group">
                                <label class="type-option {{ $settings->commission_type === 'percentage' ? 'active' : '' }}" id="label-percentage">
                                    <input type="radio" name="commission_type" value="percentage"
                                           {{ $settings->commission_type === 'percentage' ? 'checked' : '' }}
                                           onchange="switchCommissionType('percentage')">
                                    <i class="fas fa-percent me-1"></i>Percentage
                                </label>
                                <label class="type-option {{ $settings->commission_type === 'flat' ? 'active' : '' }}" id="label-flat">
                                    <input type="radio" name="commission_type" value="flat"
                                           {{ $settings->commission_type === 'flat' ? 'checked' : '' }}
                                           onchange="switchCommissionType('flat')">
                                    <i class="fas fa-coins me-1"></i>Flat Rate
                                </label>
                            </div>
                            @error('commission_type')<div class="invalid-feedback d-block" style="font-size:0.65rem; color: var(--danger);">{{ $message }}</div>@enderror
                        </div>

                        <!-- Percentage Input -->
                        <div id="input-percentage" class="{{ $settings->commission_type !== 'percentage' ? 'd-none' : '' }} mb-3">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="number" name="commission_percentage" step="0.01" min="0" max="100"
                                       ...
                                       value="{{ old('commission_percentage', $settings->commission_type === 'percentage' ? $settings->commission_value : '') }}"
                                       placeholder="e.g. 10">
                                <span class="input-group-text">%</span>
                            </div>
                            @error('commission_percentage')<div class="invalid-feedback d-block" style="font-size:0.65rem; color: var(--danger);">{{ $message }}</div>@enderror
                            <small class="text-muted">Agent earns this % of each completed order value.</small>
                        </div>

                        <!-- Flat Rate Input -->
                        <div id="input-flat" class="{{ $settings->commission_type !== 'flat' ? 'd-none' : '' }} mb-3">
                            ...
                            <div class="input-group input-group-sm">
                                ...
                                <input type="number" name="commission_flat" step="0.01" min="0"
                                       class="form-control @error('commission_flat') is-invalid @enderror"
                                       value="{{ old('commission_flat', $settings->commission_type === 'flat' ? $settings->commission_value : '') }}"
                                       placeholder="e.g. 25.00">
                            </div>
                            @error('commission_flat')<div class="invalid-feedback d-block" style="font-size:0.65rem; color: var(--danger);">{{ $message }}</div>@enderror
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group input-group-sm">
                                ...
                                <input type="number" name="min_withdrawal_amount" step="0.01" min="0"
                                       ...
                                       value="{{ old('min_withdrawal_amount', $settings->min_withdrawal_amount ?? 50) }}"
                                       placeholder="e.g. 50.00">
                            </div>
                            @error('min_withdrawal_amount')<div class="invalid-feedback d-block" style="font-size:0.65rem; color: var(--danger);">{{ $message }}</div>@enderror
                        ...
                        <div class="setting-row mb-2">
                            ...
                            <div class="form-check form-switch ms-3 mt-1">
                                <input class="form-check-input" type="checkbox" name="enable_public_signup" id="publicSignup" value="1"
                                       {{ ($settings->enable_public_signup ?? false) ? 'checked' : '' }}
                                       onchange="toggleSignupUrl(this)">
                            </div>
                        </div>

                        <!-- Signup URL (visible when toggle is on) -->
                        <div id="signupUrlBox" class="{{ ($settings->enable_public_signup ?? false) ? '' : 'd-none' }} mb-3">
