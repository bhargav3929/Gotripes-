...
@section('page-title', 'Edit Travel Package')
            ...
            <div class="card shadow-lg border-0">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center gap-3">
                        <h3 class="card-title"><i class="fas fa-suitcase-rolling me-2"></i>Edit Travel Package</h3>
                ...
                <form method="POST" action="{{ route('admin.packages.update', $package->id) }}" enctype="multipart/form-data" id="packageForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12 col-lg-8">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="title" value="{{ old('title', $package->title) }}" required maxlength="255">
                                </div>
                                    ...
                                    <div class="col-12 col-sm-6">
                                        ...
                                        <select name="country" class="form-control" required>
                                            ...
                                            @foreach($allCountries as $c)
                                                <option value="{{ $c['name'] }}" @selected(old('country', $package->country) === $c['name'])>{{ $c['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-12 col-sm-6">
                                        <label class="form-label fw-semibold text-gold">Duration <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control" name="duration" value="{{ old('duration', $package->duration) }}" required maxlength="255">
                                    </div>
                                </div>

                                <div class="mb-3 mt-3">
                                    <label class="form-label fw-semibold text-gold">Description <span class="text-danger">*</span></label>
                                    <div id="descEditor" style="height:220px;">{!! old('description', $package->description) !!}</div>
                                    <textarea name="description" id="descInput" class="d-none">{{ old('description', $package->description) }}</textarea>
                                </div>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row g-3">
                                            <div class="col-12 col-sm-3">
                                                ...
                                                <input type="number" class="form-control" name="price" value="{{ old('price', $package->price) }}" step="0.01" min="0" required>
                                            </div>
                                            <div class="col-4 col-sm-3">
                                                <label class="form-label">Per Adult</label>
                                                <input type="number" class="form-control" name="price_adult" value="{{ old('price_adult', $package->price_adult) }}" step="0.01" min="0">
                                            </div>
                                            <div class="col-4 col-sm-3">
                                                <label class="form-label">Per Child</label>
                                                <input type="number" class="form-control" name="price_child" value="{{ old('price_child', $package->price_child) }}" step="0.01" min="0">
                                            </div>
                                            <div class="col-4 col-sm-3">
                                                <label class="form-label">Per Infant</label>
                                                <input type="number" class="form-control" name="price_infant" value="{{ old('price_infant', $package->price_infant) }}" step="0.01" min="0">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card bg-light-dark border-gold">
                                    <div class="card-body">
                                        <h6 class="text-gold mb-3"><i class="fas fa-handshake me-2"></i>Local Partner Contact</h6>
                                        <div class="row g-3">
                                            <div class="col-12 col-sm-6">
                                                <label class="form-label">Partner Email</label>
                                                <input type="email" class="form-control" name="partner_email" value="{{ old('partner_email', $package->partner_email) }}" maxlength="255">
                                            </div>
                                            <div class="col-12 col-sm-6">
                            ...
                            <div class="col-12 col-lg-4">
                                <div class="card bg-light-dark border-gold mb-3">
                                    <div class="card-body">
                                        ...
                                        <select name="package_type" class="form-control" required>
                                            <option value="enquire" @selected(old('package_type', $package->package_type)==='enquire')>Enquire — customer contacts you</option>
                                            <option value="purchase" @selected(old('package_type', $package->package_type)==='purchase')>Purchase — customer pays online</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="card bg-light-dark border-gold mb-3">
                                    <div class="card-body">
                                        <h6 class="text-gold mb-2"><i class="fas fa-image me-2"></i>Cover Image</h6>
                                        @if($package->image)
                                            <img src="{{ asset($package->image) }}" class="rounded mb-2" style="max-width:100%; max-height:160px; object-fit:cover;">
                                        @endif
                                        <input type="file" class="form-control" name="image" accept="image/jpeg,image/png,image/jpg,image/gif,image/webp" onchange="previewImage(this)">
                                        <small class="text-muted">Upload to replace the current cover.</small>
                                        <div id="imagePreview" class="mt-2 d-none">
                                            <img id="previewImg" src="" class="rounded" style="max-width:100%; max-height:160px; object-fit:cover;">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card bg-light-dark border-gold">
                                    <div class="card-body">
                                        <h6 class="text-gold mb-2"><i class="fas fa-images me-2"></i>Gallery</h6>
                                        @if($package->images->count())
                                            <small class="text-muted">Tick to remove:</small>
                                            <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:6px; margin:6px 0 10px;">
                                                @foreach($package->images as $img)
                                                    <label style="position:relative; display:block; cursor:pointer;">
                                                        ...
                                                        <input type="checkbox" name="remove_images[]" value="{{ $img->id }}" style="position:absolute; top:4px; right:4px; width:18px; height:18px;">
                                                    </label>
                                                @endforeach
                                            </div>
                                        @endif
                                        <label class="form-label">Add photos <small class="text-muted">(up to 7)</small></label>
