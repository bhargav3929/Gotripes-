...
<head>
    ...
    @php
        $renewTenant = current_company();
        $renewTenantName = $renewTenant?->name ?? 'GoTrips';
        $renewTenantLogo = $renewTenant?->logo_url ?? asset('assets/index_files/logo.png');
        $renewTenantFavicon = $renewTenant?->favicon_url ?? asset('assets/index_files/logo.png');
    @endphp
    <title>Renew Trade License - {{ $renewTenantName }}</title>
    <link rel="icon" type="image/png" href="{{ $renewTenantFavicon }}">
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .header img {
            ...
            border: 2px solid #FFD700; box-shadow: 0 0 16px rgba(255, 215, 0, 0.3);
            margin-bottom: 10px;
        }
...
<div class="wrap">
    <div class="header">
        <img src="{{ $renewTenantLogo }}" alt="{{ $renewTenantName }}">
    ...
    <div class="card">
        ...
        @endif
        <form method="POST" action="{{ route('agent.renew-license.submit') }}" enctype="multipart/form-data">
            @csrf
