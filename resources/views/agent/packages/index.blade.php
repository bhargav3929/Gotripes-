...
<div class="wp-page-header">
    ...
    <a href="{{ route('agent.packages.create') }}" class="wp-btn wp-btn-primary">
...
<div class="wp-card" style="margin-bottom: 0; border-bottom: none; border-radius: 8px 8px 0 0;">
    <div style="padding: 12px 16px; display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
        ...
        @foreach($usedCountries as $c)
            <a href="{{ route('agent.packages.index', ['country' => $c]) }}"
                ...
                <tr>
                    ...
                    <td>
                        <div style="display: flex; gap: 6px;">
                            <a href="{{ route('agent.packages.edit', $package->id) }}" class="wp-btn wp-btn-secondary wp-btn-sm">
                                <i class="fas fa-pen"></i> Edit
                            </a>
                            <form action="{{ route('agent.packages.destroy', $package->id) }}" method="POST" onsubmit="return confirm('Delete this tour package?');">
                                @csrf @method('DELETE')
                ...
                <tr class="empty-row">
                    <td colspan="7">
                        <div style="padding: 24px 0; text-align: center;">
                            ...
                            <a href="{{ route('agent.packages.create') }}" style="color: var(--wp-primary);">Add your first one.</a>
                        </div>
                    </td>
                </tr>
                @endforelse
