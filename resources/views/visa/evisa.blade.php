...
<style>
    ...
    .evisa-type .t-meta { font-size: 0.74rem; color: var(--c-muted); margin-top: 2px; }
        ...
        fetch("{{ route('visa.evisa.types') }}?" + q.toString(), { headers: { 'Accept': 'application/json' } })
            ...
            .then(function (d) {
                ...
                typesBox.innerHTML = d.types.map(function (t, i) {
                    var meta = [t.entry, t.stay ? ('Stay ' + t.stay) : null, t.validity ? ('Valid ' + t.validity) : null].filter(Boolean).join(' · ');
                    return '<label class="evisa-type" data-id="' + t.id + '">' +
                        '<input type="radio" name="visa_type_id" value="' + t.id + '">' +
                        '<span><span class="t-name">' + esc(t.name) + '</span><span class="t-meta">' + esc(meta) + '</span></span>' +
